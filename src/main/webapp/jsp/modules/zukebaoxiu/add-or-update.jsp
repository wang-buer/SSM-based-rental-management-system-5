...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑租客报修</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>租客报修管理</span></li>
								<li class="breadcrumb-item active"><span>编辑租客报修</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">租客报修信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>报修日期</label>
										<div id="baoxiuriqiDate" class="input-append date form_datetime">
											<input id="baoxiuriqi-input" name="baoxiuriqi" size="15" type="text" readonly>
									...
									<div class="form-group col-md-6">
										...
										<input name="shouji" id="shouji" name="shouji" class="form-control" placeholder="手机">
											...
											<div class="form-group  col-md-12" >
												<label>问题简述</label>
												<div><div class="upload"><input id="wentijianshuupload" name="file" type="file">选择文件</div>
												<script id="wentijianshuEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var wentijianshuUe = UE.getEditor('wentijianshuEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="wentijianshu" id="wentijianshu-input">
												</div>
											</div>	
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "zukebaoxiu/info/" + id,
		...
		// 文件上传
		function upload(){
			$('#wentijianshuupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('wentijianshuEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
				...
				if(crossrefid && crossuserid) {
					...
					http("zukebaoxiu/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("zukebaoxiu/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.wentijianshu != null && ruleForm.wentijianshu != 'null' && ruleForm.wentijianshu != ''){
				var wentijianshuMes = '' + ruleForm.wentijianshu;
				var wentijianshuUeditor = UE.getEditor('wentijianshuEditor');
				wentijianshuUeditor.ready(function() {
					wentijianshuUeditor.setContent(wentijianshuMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('wentijianshuEditor').hasContents()){
				$('#wentijianshu-input').attr('value',UE.getEditor('wentijianshuEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("zukebaoxiu/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dateTimePick(){
			...
			$("#baoxiuriqi-input").val(getCurDate());
			$("#baoxiuriqiDate").datetimepicker({ 
				minView: "month",
				autoclose: true,
				format:'yyyy-mm-dd',
				language:'zh-CN',
		...
		function dataBind(){
			...
			$("#wentijianshu").val(ruleForm.wentijianshu);	
			$("#baoxiuriqi-input").val(ruleForm.baoxiuriqi);
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.wentijianshu != null && res.data.wentijianshu != ''){

						$("#wentijianshu").val(res.data.wentijianshu);
                        var wentijianshuMes = '' + res.data.wentijianshu;
                        var wentijianshuUeditor = UE.getEditor('wentijianshuEditor');
                        wentijianshuUeditor.ready(function() {
                            wentijianshuUeditor.setContent(wentijianshuMes);
                        });
					}
					if(res.data != null && res.data != '' && res.data.baoxiuriqi != null && res.data.baoxiuriqi != ''){

						$("#baoxiuriqi-input").val(res.data.baoxiuriqi);
						$('#baoxiuriqiFileName').val(res.data.baoxiuriqi)
					}
