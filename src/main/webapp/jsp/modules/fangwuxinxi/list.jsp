...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">房屋信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>房屋信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">房屋信息列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										房屋户型
									</label>
									<input type="text" id="fangwuhuxingSearch" class="form-control form-control-sm" placeholder="请输入房屋户型" aria-controls="tableId">
									<label>
										类型
									</label>
									...
									<input type="text" id="dizhiSearch" class="form-control form-control-sm" placeholder="请输入地址" aria-controls="tableId">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('fangwuleixing')">房屋类型<i id="fangwuleixingIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('fangwuhuxing')">房屋户型<i id="fangwuhuxingIcon" class="fa fa-sort"></i></th>
											...
											<th onclick="sort('zujin')">租金<i id="zujinIcon" class="fa fa-sort"></i></th>
											<th>房源设施</th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#fangwuhuxingSearch').val() != null && $('#fangwuhuxingSearch').val() != ''){
				searchForm.fangwuhuxing ="%" +  $('#fangwuhuxingSearch').val() + "%";
			}
			...
			if($('#dizhiSearch').val() != null && $('#dizhiSearch').val() != ''){
				searchForm.dizhi ="%" +  $('#dizhiSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("fangwuxinxi/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				fangwuhuxing : searchForm.fangwuhuxing,	
				leixing : searchForm.leixing,	
				dizhi : searchForm.dizhi,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var fangwuleixingCell  = document.createElement('td');

						  fangwuleixingCell.innerHTML = item.fangwuleixing; 

			row.appendChild(fangwuleixingCell);
			var fangwuhuxingCell  = document.createElement('td');

						 fangwuhuxingCell.innerHTML = item.fangwuhuxing;

			row.appendChild(fangwuhuxingCell);
			...
			tupianImg.setAttribute('src',baseUrl+item.tupian);
			...
			var zujinCell  = document.createElement('td');

						 zujinCell.innerHTML = item.zujin;

			row.appendChild(zujinCell);
			var fangyuansheshiCell  = document.createElement('td');

						  if(item.fangyuansheshi != null && item.fangyuansheshi != "" && item.fangyuansheshi.length >= 11){
			  	fangyuansheshiCell.innerHTML = item.fangyuansheshi.substring(0, 10) + "..."; 
			  }else{
			  	fangyuansheshiCell.innerHTML = item.fangyuansheshi;
			  }


			row.appendChild(fangyuansheshiCell);
			...
			var fangyuanxiangqingCell  = document.createElement('td');
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "申请租房"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'yonghuzufang','','','')");
			if(crossBtnControl('申请租房') == true){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("fangwuxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discussfangwuxinxi/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"fangwuxinxi");
